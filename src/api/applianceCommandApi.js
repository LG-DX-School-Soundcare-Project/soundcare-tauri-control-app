...
import {
  DEMO_CONTROLLER_DEVICE_ID,
  defaultCommandResponse,
  defaultControlCommands,
  withApiFallback
} from './fallbacks.js';
...
const DEFAULT_TARGET = Object.freeze({
  targetDeviceId: env.VITE_APPLIANCE_CONTROLLER_DEVICE_ID || DEMO_CONTROLLER_DEVICE_ID,
  targetModuleId: env.VITE_APPLIANCE_CONTROLLER_MODULE_ID || 'esp32s3-appliance-01',
  agentId: env.VITE_APPLIANCE_CONTROLLER_AGENT_ID || 'appliance-controller-pc-01'
});
...
export async function createControlCommand(command) {
  ...
  const created = await request('/api/control-commands', { method: 'POST', body: command })
    .catch((error) => withApiFallback(error, () => defaultCommandResponse(command), 'create control command'));
  return {
    ...command,
    ...created,
...
export async function getControlCommands(params = {}) {
  ...
  return request(`/api/control-commands${buildQuery(params)}`)
    .catch((error) => withApiFallback(error, defaultControlCommands, 'control commands'));
}
