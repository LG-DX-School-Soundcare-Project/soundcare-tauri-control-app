...
function localDataBanner() {
  if (isMockApiEnabled()) {
    return '<div class="notification-banner">Mock 모드입니다. 백엔드 또는 Agent가 실행되지 않아도 샘플 데이터로 화면을 확인할 수 있습니다.</div>';
  }
  if (isApiFallbackEnabled()) {
    return '<div class="notification-banner">API 연결 실패 시 로컬 기본값으로 화면을 표시합니다. 백엔드 또는 Agent가 꺼져 있어도 버튼/화면 테스트가 가능합니다.</div>';
  }
  return '';
}
