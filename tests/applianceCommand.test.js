...
describe('control command payload builder', () => {
  it('builds a PLAY_SAMPLE payload matching the API contract', () => {
    ...
    expect(command).toEqual({
      targetDeviceId: 'aa42b70f-9d83-4de4-969c-2e355b22c689',
...
describe('API fallback defaults', () => {
  ...
  it('accepts a control command locally when the API is unavailable', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn(() => Promise.reject(new Error('offline'))));
    const command = buildPlayCommand('robot_vacuum');

    const result = await createControlCommand(command);

    expect(result.status).toBe('PENDING');
    expect(result.commandType).toBe('PLAY_SAMPLE');
    expect(result.resultMessage).toContain('Local fallback');
  });
});
